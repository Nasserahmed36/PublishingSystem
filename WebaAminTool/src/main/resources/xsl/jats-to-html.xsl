...
<xsl:stylesheet version="1.0" exclude-result-prefixes="xlink mml"
    ...
    <xsl:variable name="doi" select="$meta/article-doi[@pub-doi-type='publisher-doi']"/>
    ...
    <xsl:template match="/article">
        <html>
            ...
            <head>
                ...
                <xsl:apply-templates select="front/title-meta" mode="head"/>
            </head>
    ...
    <xsl:template match="body//p">
        <p>
            <!-- a sequential doi for each paragraph -->
    ...
    <xsl:template name="ext-link-url">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$type = 'NCBI:refseq'">
                <xsl:value-of select="concat('https://www.ncbi.nlm.nih.gov/nuccore?term=srcdb_refseq[PROP]+AND+', $encoded-doi)"/>
            </xsl:when>
        ...
        <figcaption itemprop="description">
            ...
            <div class="figcaption-footer">
                <div class="article-image-download">
                    ...
                    <a href="{$static-root}{$fig-doi}-full.png" class="btn btn-mini" download="{$download-prefix}-{$doi}-{$fig-doi}.png" itemprop="url">
    ...
    <xsl:template match="ref-list/ref">
        <li class="{local-name()}">
            <div class="citation" itemprop="citation" itemscope="itemscope">
                <xsl:choose>
                    <xsl:when test="element-citation/@publication-type = 'title'">
    ...
    <xsl:template match="@doi | @colspan | @rowspan | @style">
        <xsl:copy-of select="."/>
    </xsl:template>
