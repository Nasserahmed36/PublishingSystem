...
<xsl:stylesheet version="1.0" exclude-result-prefixes="php"
    ...
    <xsl:variable name="related-doi" select="$meta/article-doi[@pub-doi-type='doi']"/>
    <xsl:variable name="issn" select="/article/front/title-meta/issn"/>
    <xsl:variable name="pub-date" select="concat($pub-year, '-', $pub-month, '-', $pub-day)"/>

    <xsl:template match="/article">
        <article article-type="retraction" dtd-version="1.1">
            <front>
                <xsl:apply-templates select="front/title-meta"/>
    ...
    <xsl:template match="article-meta">
        <article-meta>
            <article-doi pub-doi-type="publisher-doi">
                <xsl:value-of select="$doi"/>
            </article-doi>
            <article-doi pub-doi-type="doi">
                <xsl:value-of select="$doi"/>
            </article-doi>
            <article-categories>
            ...
            <!--<contrib-group content-type="authors">
                <contrib doi="author-1" contrib-type="author" corresp="yes">
            ...
            <elocation-doi>
                 <xsl:value-of select="concat('e', $doi)"/>
            </elocation-doi>
            <permissions>
            ...
            <related-article related-article-type="retracted-article"
                             title-doi-type="issn" title-doi="{$issn}"
                             vol="{volume}" elocation-doi="{elocation-doi}"
