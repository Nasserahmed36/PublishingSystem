...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="article-doi" select="$article-meta/article-doi[@pub-doi-type='publisher-doi']"/>
  <!-- <xsl:variable name="doi" select="$article-meta/article-doi[@pub-doi-type='doi']"/> -->
  ...
  <xsl:template match="article">
    ...
    <posted_content type ="preprint" language="en" metadata_distribution_opts="any">
      ...
      <!-- group_title -->
      <group_title>
        <xsl:value-of select="front/title-meta/title-title-group/title-title"/>

        <!-- <issn media_type="electronic">
        <xsl:value-of select="front/title-meta/issn"/>
      </issn>
...
<!-- components -->
<!-- <component_list>
<xsl:apply-templates select="body//fig[object-doi[@pub-doi-type='doi']]" mode="component"/>
<xsl:apply-templates select="body//table-wrap[object-doi[@pub-doi-type='doi']]" mode="component"/>
<xsl:apply-templates select="body//supplementary-material[object-doi[@pub-doi-type='doi']]" mode="component"/>
</component_list>
</journal_article> -->
</posted_content>
</xsl:template>
...
<xsl:template match="article-meta">
  ...
  <!-- item_number -->
  <!-- as this can be used as the page number in citations (though first_page takes precedence), use the elocation-doi with the "e" prefix rather than the actual article doi -->
  <!-- http://help.crossref.org/using_best_practices_depositing -->
  <item_number item_number_type="article-number">
    <xsl:value-of select="elocation-doi"/>
...
<xsl:template name="relatedItems">
  <xsl:if test="$isPreprintOf or $previousVersionDoi or $nextVersionDoi or //supplementary-material[object-doi[@pub-doi-type='doi']] ">
    <rel:program>
      <xsl:apply-templates select="//supplementary-material[object-doi[@pub-doi-type='doi']]" mode="related-item"/>
...
<relatedIdentifiers>
  <!--<xsl:apply-templates select="../../back/ref-list/ref/pub-doi[@pub-doi-type='doi']"/>-->
  <xsl:if test="$previousVersionDoi">
  ...
  <xsl:choose>
    <xsl:when test="name">
      <person_name contributor_role="author" sequence="additional">
        ...
        <xsl:apply-templates select="contrib-doi[@contrib-doi-type='orcid']" mode="contrib"/>
      </person_name>
    </xsl:when>
...
<xsl:template match="contrib-doi[@contrib-doi-type='orcid']" mode="contrib">
...
<xsl:template match="xref[@ref-type='aff']" mode="contrib">
  <xsl:apply-templates select="//aff[@doi=current()/@rid]" mode="contrib"/>
</xsl:template>
...
<xsl:template match="element-citation">
  ...
  <citation key="{$key}">
    ...
    <xsl:apply-templates select="fpage | elocation-doi" mode="citation"/>
    <xsl:apply-templates select="year" mode="citation"/>
    <xsl:apply-templates select="pub-doi[@pub-doi-type='doi'][1]" mode="citation"/>
...
<xsl:template match="pub-doi[@pub-doi-type='doi']" mode="citation">
  <doi>
    <xsl:apply-templates/>
  </doi>
</xsl:template>
...
<xsl:template match="elocation-doi" mode="citation">
  <first_page>
    <xsl:apply-templates/>
  </first_page>
</xsl:template>
...
<xsl:template match="fig" mode="component">
  <component parent_relation="isPartOf">
    ...
    <doi_data>
      <doi><xsl:value-of select="object-doi[@pub-doi-type='doi']"/></doi>
      <resource><xsl:value-of select="concat($url, '/', @doi)"/></resource>
    </doi_data>
  </component>
</xsl:template>
...
<xsl:template match="supplementary-material" mode="related-item">
  <rel:related_item>
    ...
    <rel:inter_work_relation relationship-type="references" identifier-type="doi">
      <xsl:value-of select="object-doi[@pub-doi-type='doi']"/>
...
<xsl:template match="funding-group/award-group/funding-source" mode="fundref">
  <fr:assertion name="fundgroup">
    ...
    <xsl:variable name="institution-doi" select="institution-wrap/institution-doi[@institution-doi-type='FundRef']"/>
    <fr:assertion name="funder_name">
    <xsl:value-of select="institution-wrap/institution"/>
  </fr:assertion>
  <xsl:if test="$institution-doi">
  <fr:assertion name="funder_identifier">
  <xsl:value-of select="$institution-doi"/>
</fr:assertion>
</xsl:if>
</xsl:when>
<xsl:otherwise>
...
<xsl:apply-templates select="../award-doi" mode="fundref"/>
</fr:assertion>
</xsl:template>

<xsl:template match="award-doi" mode="fundref">
