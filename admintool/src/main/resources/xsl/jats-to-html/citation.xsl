...
<xsl:stylesheet version="1.0" exclude-result-prefixes="xlink"
    ...
    <!-- title citation -->
    <xsl:template match="element-citation[@publication-type='title']">
        ...
        <span>
	        <xsl:choose>
		        <xsl:when test="issue">
			        <!-- if an irefrefrefssue exists, the source and volume are part of it -->
			        <xsl:apply-templates select="issue" mode="title-citation"/>
		        </xsl:when>
		        <xsl:when test="volume">
			        <!-- if a volume exists, the source is part of it -->
			        <xsl:apply-templates select="volume" mode="title-citation"/>
		        </xsl:when>
		        <xsl:otherwise>
			        <xsl:apply-templates select="source" mode="title-citation"/>
			        <xsl:text>&#32;</xsl:text>
		        </xsl:otherwise>
	        </xsl:choose>
            <xsl:apply-templates select="elocation-doi" mode="citation"/>
    ...
    <xsl:template match="element-citation[@publication-type='book']">
        ...
        <span>
            ...
            <xsl:call-template name="pagination"/>
            <xsl:text>&#32;</xsl:text>
            <xsl:apply-templates select="pub-doi[@pub-doi-type='isbn']" mode="citation"/>
            <xsl:call-template name="comment"/>
        </span>
    </xsl:template>
    ...
    <!-- title name -->
	<xsl:template match="source" mode="title-citation">
		<span class="{local-name()}"
    ...
    <xsl:template name="citation-url">
        ...
        <xsl:variable name="doi" select="$citation/pub-doi[@pub-doi-type='doi']"/>
        <xsl:variable name="arxiv" select="$citation/pub-doi[@pub-doi-type='arxiv']"/>
                ...
                <xsl:choose>
                    <xsl:when test="$citation/@publication-type = 'book'">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of select="concat('#', $citation/../@doi)"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                    <xsl:otherwise>
	...
	<xsl:template match="volume" mode="title-citation">
		<span class="{local-name()}"
		      itemprop="isPartOf" itemscope="itemscope"
	...
	<xsl:template match="issue" mode="title-citation">
		<span class="{local-name()}"
		      itemprop="isPartOf" itemscope="itemscope"
