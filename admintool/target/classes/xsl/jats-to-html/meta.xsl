...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="article-meta" mode="head">
        ...
        <xsl:choose>
            <xsl:when test="$publication-type = 'publication'">
                ...
                <meta name="citation_firstpage" content="{elocation-doi}"/>
            </xsl:when>
            <xsl:when test="$publication-type = 'preprint'">
                <!-- note: no citation_volume for preprints -->
                <meta name="citation_technical_report_number" content="{elocation-doi}"/>
            </xsl:when>
        </xsl:choose>
    ...
    <!-- "meta" tags, title -->
    <xsl:template match="title-meta" mode="head">
        <xsl:choose>
            <xsl:when test="$publication-type = 'publication'">
                <meta name="citation_journal_title" content="{$title-title}"/>
                <meta name="citation_journal_abbrev" content="{$abbrev-title-title}"/>
            </xsl:when>
            <xsl:when test="$publication-type = 'preprint'">
                <meta name="citation_technical_report_institution" content="{$title-title}"/>
