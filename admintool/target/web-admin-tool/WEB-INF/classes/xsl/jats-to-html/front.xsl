...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="contrib" mode="front">
            ...
            <xsl:if test="xref">
                <xsl:variable name="xrefs">
                    <xsl:for-each select="xref[not(@ref-type='corresp')]">
                        ...
                        <xsl:variable name="ref" select="//*[@doi=$rid]"/>
        ...
        <!--
            <xsl:variable name="contributorId" select="@doi"/>
    ...
    <xsl:template match="*" mode="contrib-xref">
        <a class="{local-name()} xref" href="#{@doi}">
            <xsl:if test="local-name() = 'aff'">
                ...
                <xsl:attribute name="itemref"><xsl:value-of select="@doi"/></xsl:attribute>
            </xsl:if>
